...
export default function SearchingArea({setCarName, setBrandName, setIsClickFind}: SearchingAreaProps) {
  ...
  useEffect(() => {
    if (choosenVehicle)
      setVehicleSearchField(choosenVehicle);
  }, [choosenVehicle])
  ...
  useEffect(() => {
    const fetchData = async() => {
      ...
      if (brandSearchField) {
        const data = await fetchRecommendedCarNamesByBrand(brandSearchField, debouncedVehicleNameSearch);
  ...
  const handleMainInputClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const findBrand = async () => {
      brandName = await getBrandNameByVehicleName(event.currentTarget.textContent);
    ...
    >
      <div className={`w-screen h-80 py-14 flex-grow bg-gray-200 ${!isMore ? "pb-48 lg:pb-24" : ""}  dark:bg-gray-900`}>
        ...
        <div className="lg:w-3/4 w-10/12 lg:flex mx-auto relative justify-between">
          ...
          <div className={`absolute ${isMainSearchRecommend ? "hidden lg:flex" : ""} inset-y-0 left-0 flex items-center pl-3 pointer-events-none`}>
            <svg
        ...
        <div className="flex justify-between w-3/4 relative mx-auto pt-3">
          {
            !isMore && (
                      ...
                      if (index > 3) {
                        ...
                        return (
                          <button
                            ...
                            key={index}>{vehicle.toUpperCase()}</button>
                        )
                      }
                    }
                  })
